...
function validate(payload) {
  ...
  if (!payload.password && !payload.id) errors.password = 'Password is required'
  return errors
}

export default function UserForm({ user, onClose, onSave }) {
  const [payload, setPayload] = useState({ username: '', email: '', password: '', roles: [] })
          ...
          <div className="modal-content">
            <form onSubmit={handleSubmit}>
              ...
              <div className="modal-body">
                ...
                <div className="mb-3">
                  <label className="form-label">Password {user ? <small className="text-muted">(leave blank to keep)</small> : null}</label>
                  <input type="password" className={`form-control ${errors.password ? 'is-invalid' : ''}`} value={payload.password} onChange={e => handleChange('password', e.target.value)} />
